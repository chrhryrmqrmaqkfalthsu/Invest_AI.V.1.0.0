...
Isolated experiment script. Production code, manifest, positions and trade logs are
not modified. The script reuses the production GA and backtest functions but
writes only to data/_system/research/exp_downturn/*.
"""
...
def _pct(trade: Mapping[str, Any], key: str) -> float | None:
    entry = _num(trade.get("entry_price"))
    value = trade.get(key)
    if not entry or value is None:
        return None
    return (_num(value) - entry) / entry * 100.0
...
def trade_quality(result: Any) -> dict[str, Any]:
    ...
    stops = [abs(x) for t in trades if (x := _pct(t, "stop_price_at_entry")) is not None]
    trails = [abs(x) for t in trades if (x := _pct(t, "trailing_stop_at_entry")) is not None]
    targets = [x for t in trades if (x := _pct(t, "target_price_at_entry")) is not None]
    mfe5_bad = [
        t
        for t in trades
...
def evaluate_best(rb: Rulebook, ctx: Mapping[str, Any], *, train_start: str, train_end: str, oos_end: str) -> dict[str, Any]:
...
def run_condition(*, ticker: str, condition: str, ctx: Mapping[str, Any], train_start: str, train_end: str, oos_end: str) -> dict[str, Any]:
    ...
    started = time.time()
    ga_result = run_ga(base_rulebook=ctx["base_rulebook"], evaluate_fn=evaluator, ga_config=ga_cfg)
    ...
    return {
        ...
        "composite_ga_fitness": safe_float(getattr(best, "fitness", 0.0), 0.0),
        "market_genes": market_genes(best),
        "evaluations": evaluate_best(best, ctx, train_start=train_start, train_end=train_end, oos_end=oos_end),
        "ga_config": {
...
def run_experiment(args: argparse.Namespace) -> int:
        ...
        for ticker in args.tickers:
            ...
            for condition in ("A", "B"):
                ...
                row.update({
                    "train_start": train_start,
                    "train_end": train_end,
                    "downturn_start": DOWN_START,
                    "downturn_end": DOWN_END,
                    "oos_start": OOS_START,
                    "oos_end": oos_end,
                })
                rows.append(row)
                ...
                safe_emit(
                    "|".join(map(str, [
                        ticker,
                        condition,
                        "use_adj", row["market_genes"]["use_market_entry_adjustment"],
                        "eff", row["market_genes"]["effective_market_score_sensitivity"],
                        "2022_trades", row["evaluations"]["downturn_2022"]["trade_count"],
                        "2022_exp", round(row["evaluations"]["downturn_2022"]["expectancy_pct"], 4),
                        "2022_dd", round(row["evaluations"]["downturn_2022"]["max_drawdown_pct"], 4),
                        "2025H2_exp", round(row["evaluations"]["oos_2025h2"]["expectancy_pct"], 4),
                    ]))
                )
        status = "DONE"
        error = None
    ...
    safe_emit(json.dumps({k: v for k, v in summary.items() if k != "rows"}, ensure_ascii=False, indent=2, sort_keys=True, default=str))
    return return_code

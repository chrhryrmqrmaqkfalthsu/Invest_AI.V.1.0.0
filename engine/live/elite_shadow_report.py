"""Read-only elite shadow candidate report builder."""
...
def load_candidate_denylist(path: Path = DENYLIST_PATH) -> dict[str, Any]:
    payload = {"enabled": False, "path": str(path), "version": None, "entries": [], "error": None}
    if not path.exists():
        return payload
    try:
...
def _denylist_entry_matches(candidate: dict[str, Any], entry: dict[str, Any]) -> bool:
    ...
    return not entry_stage or entry_stage == candidate_stage
...
def apply_candidate_denylist(candidates: list[dict[str, Any]], *, path: Path = DENYLIST_PATH) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for candidate in candidates:
        matched = next((entry for entry in entries if _denylist_entry_matches(candidate, entry)), None)
        if matched is None:
        ...
        summary["blocked"].append({
            ...
            "reason_label": matched.get("reason_label") or matched.get("reason") or "candidate_denylist",
        })
...
def _metrics_summary(metrics: dict[str, Any]) -> dict[str, Any]:
    labels = ["train_1_eval", "train_2_eval", "train_3_eval", "oos_2025h2", "stress_pre_2022h1"]
...
def _elite_score(metrics: dict[str, Any], rulebook: dict[str, Any]) -> float:
    ...
    return (
        ...
        + dd_bonus + exit_bonus + omen_bonus + hold_bonus + adj_bonus
    )
...
def _rulebook_passes_anti_pattern_filter(rulebook: dict[str, Any], metrics: dict[str, Any], *, stage: str) -> tuple[bool, str]:
    ...
    if exp < 2.7: return False, "rulebook_expectancy_lt_2.7"
    if fit < (70.0 if stage == "stage2" else 45.0): return False, "rulebook_fitness_too_low"
    if not _is_market_adj_ok(rulebook, metrics): return False, "market_adjustment_too_weak"
    if str(rulebook.get("exit_strategy") or "") == "fixed" and hold > 19: return False, "fixed_exit_long_holding"
    if hold > 24: return False, "holding_days_gt_24"
    if target_atr and target_atr < 1.2: return False, "target_too_small"
    if stop_atr and stop_atr > 3.5 and target_atr and target_atr < 2.5: return False, "bad_target_stop_shape"
    return True, "pass"
...
def _stage_sort_key(item: dict[str, Any]):
    return (item["elite_score"], item["metrics"]["oos_fitness"], item["metrics"]["oos_expectancy_pct"])
...
def collect_stage2_elite(*, max_unique: int = 60) -> tuple[list[dict[str, Any]], Counter]:
    ...
    for row in _load_jsonl(CENTRAL_INDEX):
        if not row: continue
        if not row.get("eligible", True): skipped["not_eligible"] += 1; continue
        if row.get("stage") != "stage2": skipped["not_stage2"] += 1; continue
        metrics = _metrics_summary(row.get("metrics") or {})
        if metrics["oos_expectancy_pct"] < 2.7: skipped["oos_expectancy_lt_2.7"] += 1; continue
        if metrics["oos_fitness"] < 70.0: skipped["oos_fitness_lt_70"] += 1; continue
        if metrics["oos_trade_count"] < 15: skipped["oos_trades_lt_15"] += 1; continue
        if metrics["oos_win_rate"] < 70.0: skipped["oos_win_lt_70"] += 1; continue
        if metrics["stress_expectancy_pct"] < 0.5: skipped["stress_expectancy_lt_0.5"] += 1; continue
        if metrics["worst_drawdown_pct"] <= -18.0: skipped["worst_drawdown_lte_-18"] += 1; continue
        if metrics["min_trade_count"] < 8: skipped["min_trades_lt_8"] += 1; continue
        source = _load_source_row(row.get("source_file"), _safe_int(row.get("source_row_index")))
        if not source: skipped["source_row_missing"] += 1; continue
        ok, reason = _rulebook_passes_anti_pattern_filter(source.get("rulebook") or {}, metrics, stage="stage2")
        if not ok: skipped[reason] += 1; continue
        rows.append(_stage2_candidate_from_row(row, source))
    out, shadow = apply_upstream_gate_shadow(rows, stage="stage2", max_unique=max_unique, sort_key=_stage_sort_key)
    skipped["upstream_shadow_fail"] += int((shadow.get("decision_counts") or {}).get("FAIL", 0))
    skipped["upstream_shadow_hold"] += int((shadow.get("decision_counts") or {}).get("HOLD", 0))
    return out, skipped
...
def collect_stage3_elite(*, max_unique: int = 80) -> tuple[list[dict[str, Any]], Counter]:
    ...
    for path in TICKERS_ROOT.glob("*/stage3/final_rulebooks.jsonl"):
        ...
        for row in _load_jsonl(path):
            ...
            if metrics["oos_expectancy_pct"] < 2.7: skipped["expectancy_lt_2.7"] += 1; continue
            if metrics["oos_fitness"] < 45.0: skipped["fitness_lt_45"] += 1; continue
            if metrics["oos_win_rate"] < 70.0: skipped["win_lt_70"] += 1; continue
            if metrics["oos_trade_count"] < 8: skipped["trades_lt_8"] += 1; continue
            if metrics["worst_drawdown_pct"] <= -18.0: skipped["drawdown_lte_-18"] += 1; continue
    ...
    out, shadow = apply_upstream_gate_shadow(rows, stage="stage3", max_unique=max_unique, sort_key=_stage_sort_key)
    skipped["upstream_shadow_fail"] += int((shadow.get("decision_counts") or {}).get("FAIL", 0))
    skipped["upstream_shadow_hold"] += int((shadow.get("decision_counts") or {}).get("HOLD", 0))
    return out, skipped
...
def _summarize_trades(trades: list[dict[str, Any]]) -> dict[str, Any]:
    if not trades:
        return {"trade_count": 0, "win_count": 0, "loss_count": 0, "win_rate": 0.0, "avg_pnl_pct": 0.0, "sum_pnl_pct": 0.0, "compounded_pct": 0.0, "avg_mfe_pct": 0.0, "avg_mae_pct": 0.0, "best_trade_pct": 0.0, "worst_trade_pct": 0.0}
    pnls = [_safe_float(t.get("pnl_pct")) for t in trades]
    wins = [p for p in pnls if p > 0]
    losses = [p for p in pnls if p <= 0]
    compounded = 1.0
    for pnl in pnls: compounded *= 1.0 + pnl / 100.0
    mfes = [_safe_float(t.get("max_profit_during_hold")) for t in trades]
    maes = [_safe_float(t.get("max_loss_during_hold")) for t in trades]
    return {
        "trade_count": len(trades), "win_count": len(wins), "loss_count": len(losses),
        "win_rate": len(wins) / len(trades) * 100.0, "avg_pnl_pct": sum(pnls) / len(pnls),
        "sum_pnl_pct": sum(pnls), "compounded_pct": (compounded - 1.0) * 100.0,
        "avg_mfe_pct": sum(mfes) / len(mfes) if mfes else 0.0,
        "avg_mae_pct": sum(maes) / len(maes) if maes else 0.0,
        "best_trade_pct": max(pnls), "worst_trade_pct": min(pnls),
    }
    ...
    for candidate in candidates:
        ...
        if trade_path and trade_path.exists():
            for row in _load_jsonl(trade_path):
                if str(row.get(hash_key) or "") != rulebook_hash: continue
                trades.append(_normalize_trade(row, stage=stage))
                if len(trades) >= max_trades_per_candidate: break
...
def build_elite_shadow_report(*, stage2_limit: int = 60, stage3_limit: int = 80, include_trades: bool = True) -> dict[str, Any]:
    ...
    return {
        "_comment": "Read-only elite shadow report. Upstream v3/BOIL defaults to SHADOW and does not remove candidates.",
        ...
        "filters": {
            ...
            "candidate_denylist": "data/_system/candidate_denylist.json",
            "upstream_gate": "v3 + final BOIL-exclusive; pre-ticker-dedup; default SHADOW",
        },
        "summary": {

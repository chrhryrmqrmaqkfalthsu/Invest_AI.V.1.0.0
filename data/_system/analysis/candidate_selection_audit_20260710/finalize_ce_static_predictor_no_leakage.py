...
"""CE 정적 predictor 최종 무누수 검증.

타깃·성과 파생 컬럼(eval_*, target_*, IS-OOS gap)은 predictor 후보에서 강제 제외한다.
"""
...
def pure_static_features(discovery: pd.DataFrame, validation: pd.DataFrame) -> list[str]:
    candidates = v2.feature_columns(discovery, validation)
    forbidden_prefixes = ("eval_", "target_", "is_oos_", "oos_to_is_")
    forbidden_exact = {
        "holdout_n", "holdout_avg_pnl_pct", "holdout_win_rate_pct",
        "base_n", "base_avg_pnl_pct", "base_win_rate_pct",
        "all_history_n", "all_history_avg_pnl_pct", "all_history_win_rate_pct",
    }
    return [
...
def main() -> int:
    ...
    features = pure_static_features(discovery, validation)
    ...
    prior_summary = json.loads(core.SUMMARY_OUT.read_text(encoding="utf-8"))
    summary = {
        "created_at": datetime.now(timezone.utc).isoformat(),
        "analysis_version": "v5-relative-outcome-no-leakage-final",
        "verdict": verdict,
        "method": {
            "discovery_n": len(discovery),
            "validation_n": len(validation),
            "target": "split/stage-relative collapse, sample-adjusted tail, high-win large-loss",
            "predictor_threshold": "discovery only; frozen no retuning",
            "pure_static_feature_rule": "exclude all eval_*, target_*, IS-OOS outcome gap and raw history outcome columns",
        ...
        "target_thresholds": prior_summary.get("target_thresholds", {}),
        "cohorts": {
        ...
        "leakage_audit": {
            "detected_and_removed": [
                "eval_worst_mae_sample_adjusted_residual_pct",
                "eval_worst_mae_expected_for_n_pct",
                "all eval_* outcome fields",
                "target_* labels",
                "is_oos_gap_pp",
                "oos_to_is_pnl_ratio",
            ],
            "leaked_preliminary_result_used_for_verdict": False,
        },
        "conclusion": (
            "A pure rulebook-static predictor survived frozen bootstrap and FDR."
    ...
    lines = [
        ...
        "",
        f"최종 검증 특징은 순수 정적 {len(power)}개, 2-feature 조합 {len(pairs)}개다.",
        "",
        "## 3. CE 7개 타깃 타당성",
        "",

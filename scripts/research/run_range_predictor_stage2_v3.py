...
def clone_individual(ind: PredictorIndividual) -> PredictorIndividual:
    return PredictorIndividual(
        [RuleGene(**asdict(r)) for r in ind.rules],
        int(ind.default_high_bin),
        int(ind.default_low_bin),
        json.loads(json.dumps(json_safe(ind.baseline_spec))),
        float(ind.fitness),
        json.loads(json.dumps(json_safe(ind.metrics))) if ind.metrics is not None else None,
        ind.signature,
    )
...
def baseline_metrics(df: pd.DataFrame, spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {"exact_baseline": exact, "adjacent_baseline": adjacent}
...
def crossover(a: PredictorIndividual, b: PredictorIndividual, rng: random.Random, baseline_spec: dict[str, Any]) -> PredictorIndividual:
    return PredictorIndividual(
        [RuleGene(**asdict(ra if rng.random() < 0.5 else rb)) for ra, rb in zip(a.rules, b.rules)],
...
def score_period_candidates(candidates: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    ar = percentile_ranks([safe_float(r.get("combined_adjacent_lift_pp")) for r in rows])
    ...
    for i, row in enumerate(rows):
        ...
        r["member_score_components"] = {"exact_lift_percentile": round(er[i], 6), "adjacent_lift_percentile": round(ar[i], 6), "low_penalty_percentile": round(pr[i], 6)}
        out.append(r)
    return out
...
def period_family(kind: str) -> str:
    k = str(kind or "").lower()
    if k == "stress" or "stress" in k:
        return "stress"
    if k == "oos" or "oos" in k:
        return "oos"
    return "train"
...
def fail_reasons(metrics: Mapping[str, Any], kind: str, config: GateConfig = DEFAULT_GATE) -> list[dict[str, Any]]:
    ...
    checks = [
        ("sample_count", safe_int(metrics.get("sample_count")), config.min_samples, ">="),
        ("member_score", safe_float(metrics.get("member_score")), config.min_member_score, ">="),
        ("combined_exact_lift_pp", safe_float(metrics.get("combined_exact_lift_pp")), min_exact, ">="),
        ("combined_adjacent_lift_pp", safe_float(metrics.get("combined_adjacent_lift_pp")), min_adj, ">="),
        ("total_penalty", safe_float(metrics.get("total_penalty")), config.max_total_penalty, "<="),
        ("max_pred_share_pct", max_share, config.max_pred_share_pct, "<="),
    ]
    out = []
...
def evaluate_population(pop: list[PredictorIndividual], df: pd.DataFrame, features: list[str], qspec: dict[str, dict[str, list[float]]], label: str, kind: str) -> list[dict[str, Any]]:
    ...
    for row in scored:
        row["fail_reasons"] = fail_reasons(row, kind)
...
def select_survivors(pop: list[PredictorIndividual], scored_rows: list[dict[str, Any]], survivor_count: int) -> tuple[list[PredictorIndividual], list[dict[str, Any]]]:
    ...
    for row in selected_rows:
        ...
        if sig in by_sig and sig not in seen:
            ind = clone_individual(by_sig[sig])
            ind.fitness = safe_float(row.get("fitness")); ind.metrics = dict(row); ind.signature = sig
            survivors.append(ind); seen.add(sig)
...
def build_final_periods(data: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    out = []
    for p in FINAL_PERIODS_TEMPLATE:
        row = dict(p)
...
def run_sequential_stage2_predictor(ticker: str, out_dir: Path, seed_base: int, survivor_count: int) -> dict[str, Any]:
    ...
    for split_idx, split in enumerate(TRAIN_SPLITS, 1):
        ...
        for row in scored:
            r = dict(row); r.update({"ticker": ticker, "stage": split_idx, "train_start": split["train_start"], "train_end": split["train_end"]})
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--survivor-count", type=int, default=SURVIVOR_COUNT)
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_sequential_stage2_predictor(ticker, out_dir, seed_base, survivor_count)
    return 0

    ...
    *,
    ...
    for day in days:
        ...
        for decision in decisions:
            _execute_buy(day, decision, ledger, broker, provider, entity_by_id[decision.entity_id], rb_by_entity[decision.entity_id], result)
...
def _process_exits(day, ledger, broker: SimBroker, provider: CacheOnlyDataProvider, entity_by_id: dict, rb_by_entity: dict, result: BacktestResult) -> None:
    for pos in list(ledger.open_positions()):
        ...
        execution = ledger.dispatch_execution(intent.intent_id, broker, _client_order_id("sell", pos.entity_id, pos.ticker, snap.date))
...
def _execute_buy(day, decision, ledger, broker: SimBroker, provider: CacheOnlyDataProvider, entity: EntityRecord, rb: Rulebook, result: BacktestResult) -> None:
    shares = _cash_capped_shares(broker, decision.ticker, decision.shares)
    if shares <= 0.0:
        return
    intent = ledger.open_intent(
        decision.entity_id,
    ...
    execution = ledger.dispatch_execution(intent.intent_id, broker, _client_order_id("buy", decision.entity_id, decision.ticker, pd.Timestamp(day).strftime("%Y-%m-%d")))
    order = broker.get_order(execution.order_id)
    if order is None:
        return
    ledger.apply_fill(execution.execution_id, order)
...
def _cash_capped_shares(broker: SimBroker, ticker: str, requested_shares: float) -> float:
    requested = normalize_shares(requested_shares)
    if requested <= 0.0:
        return 0.0
    price, _ = broker.execution_price(ticker, OrderSide.BUY)
    if price <= 0.0:
        return 0.0
    max_affordable = broker.cash / price
...
def _initialize_record_from_entry(pos, entity: EntityRecord, rb: Rulebook, provider: CacheOnlyDataProvider, order, signal_day) -> None:
    ...
    idx = _index_for_date(df, order.filled_at or signal_day)
    if idx is None:

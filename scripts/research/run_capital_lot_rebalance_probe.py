...
def main() -> int:
    parser = argparse.ArgumentParser(description="Lot-level signal-proportional cap-binding rebalance probe")
    ...
    parser.add_argument("--dry-run", action="store_true", help="Validate stage2 artifacts and print plan only; no full simulation.")
    args = parser.parse_args()

    limit = int(args.limit or 0) or None
    if args.dry_run:
        summary = dry_run_plan(Path(args.trades_jsonl), Path(args.ohlcv_cache), limit=limit)
    else:

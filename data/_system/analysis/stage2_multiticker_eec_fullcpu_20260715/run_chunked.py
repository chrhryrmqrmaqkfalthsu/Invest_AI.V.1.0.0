...
import argparse, copy, hashlib, json, os, random, sys, time, math
...
def ctx(repo,stage,ticker):
 ...
 if raw.index.isna().any() or raw.isna().any().any(): raise RuntimeError(f'invalid OHLCV {ticker}')
...
def evchunk(chunk):
 return [_eval_one(i,rb) for i,rb in chunk]
def chunks(pop,n):
 n=max(1,min(n,len(pop))); out=[[] for _ in range(n)]
 for i,rb in enumerate(pop): out[i%n].append((i,rb))
 return [c for c in out if c]
def evalpop(ex,pop,workers):
 fut=[ex.submit(evchunk,c) for c in chunks(pop,workers)]
 for f in as_completed(fut):
  for i,fit in f.result(): pop[i].fitness=fit
...
def ga(base,df,kw,sp,seed,workers,log):
 ...
 with ProcessPoolExecutor(max_workers=workers,initializer=init,initargs=(str(s.PROJECT_ROOT),df,kw,sp)) as ex:
  evalpop(ex,pop,workers); bestall=copy.deepcopy(max(pop,key=lambda r:r.fitness))
  for gen in range(1,s.GENERATIONS+1):
   pop.sort(key=lambda r:r.fitness,reverse=True); best=pop[0]; avg=float(np.mean([r.fitness for r in pop])); hist.append((gen,best.fitness,avg)); log.info('gen=%s split=%s best=%.6f avg=%.6f chunks=%s chunk_size≈%.2f',gen,sp['label'],best.fitness,avg,min(workers,len(pop)),len(pop)/max(1,min(workers,len(pop))))
   ...
   while len(new)<s.POPULATION:
    ...
    new.append(child)
   pop=new; evalpop(ex,pop,workers)
...
def main():
 p=argparse.ArgumentParser(); p.add_argument('--repo-root',default='.'); p.add_argument('--out-dir',required=True); p.add_argument('--ticker',required=True); p.add_argument('--seed-base',type=int,default=2026071401); p.add_argument('--workers',type=int,default=28); a=p.parse_args()
 ...
 launch={'host':__import__('socket').gethostname(),'python':sys.executable,'ticker':a.ticker.upper(),'workers':a.workers,'actual_worker_processes':a.workers,'parallel_unit':'GA population chunk evaluation','chunks_per_generation':min(a.workers,s.POPULATION),'approx_rulebooks_per_chunk':s.POPULATION/max(1,min(a.workers,s.POPULATION)),'seed_base':a.seed_base,'ohlcv_sha256':c['cache_only_metadata']['ohlcv_cache_sha256'],'run_stage2_sha256':sha(stage/'scripts/research/run_stage2.py'),'runner_sha256':sha(Path(__file__))}
 (out/'launch_command.json').write_text(json.dumps(launch,ensure_ascii=False,indent=2),encoding='utf-8')
 trs=[]; rows=[]; hist=[]
 for i,sp in enumerate(s.TRAIN_SPLITS,1):
  log.info('chunked train start ticker=%s split=%s workers=%s chunks=%s',a.ticker.upper(),sp['label'],a.workers,min(a.workers,s.POPULATION)); tr=train(a.ticker.upper(),i,sp,c,a.seed_base,a.workers,log); trs.append(tr); rows+=tr['rows']; hist+=tr['history']; log.info('chunked train done ticker=%s split=%s gen=%s elapsed=%.1f',a.ticker.upper(),sp['label'],tr['generations_run'],tr['elapsed'])
